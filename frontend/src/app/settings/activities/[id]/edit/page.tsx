...
export default function EditActivityTypePage() {
  ...
  const fetchActivityType = async () => {
    try {
      const response = await axios.get(`http://localhost:3001/api/activity-types/${activityId}`);
      ...
      alert('ไม่สามารถโหลดข้อมูล Activity Type ได้');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await axios.patch(`http://localhost:3001/api/activity-types/${activityId}`, payload);

      alert('แก้ไข Activity Type สำเร็จ');
      router.push('/settings/activities');
    } catch (error: any) {
      console.error('Error updating activity type:', error);
      alert(error.response?.data?.message || 'ไม่สามารถแก้ไข Activity Type ได้');
    } finally {
      setSaving(false);
    }
  };
  ...
  if (loading) {
    return (
      <MainLayout
        title="แก้ไข Activity Type"
        subtitle="Edit Activity Type"
        showBackButton={true}
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-sm border border-border p-6 space-y-6">
          ...
          <div className="flex items-center gap-3 p-4 bg-gray-50 rounded-lg">
            ...
            <label htmlFor="isActive" className="text-sm font-medium text-foreground cursor-pointer">
              ...
              <p className="text-xs text-muted-foreground mt-1">กิจกรรมนี้สามารถใช้งานได้หรือไม่</p>
            </label>
          </div>
